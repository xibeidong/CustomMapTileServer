...
func getPositionsByDb(pb *playBackPositions)  {
	...
	tableName1:=fmt.Sprintf("gpainfo%02d%02d", begin.Year(), begin.Month())
	end, err := time.Parse(common.TimeFormat, pb.TEnd)
	if err!=nil{
		zapLog.Logger.Error(err)
		return
	}
	tableName2:=fmt.Sprintf("gpainfo%02d%02d", end.Year(), end.Month())

	var sqlStr string
	if end.Month() == begin.Month() {
		sqlStr = fmt.Sprintf("select * from %s where t> '%s' and t>'%s'",tableName1,
		pb.TBegin,pb.TEnd)
	}else{
		sqlStr = fmt.Sprintf("(select * from %s where t> '%s' and t>'%s') union (select * from %s where t> '%s' and t>'%s')",tableName1,
			pb.TBegin,pb.TEnd,tableName2,pb.TBegin,pb.TEnd)
	}
